...
class StudentSerializer(serializers.ModelSerializer):
    ...
    def get_courses(self, instance):
        ...
        """
        queryset = [
            sac.course for sac in StudentAttendCourses.objects.filter(student=instance)]
        print(queryset)
        queryset = [{'id': item.id,
    ...
    def get_lecture_done(self, instance):
        ...
        lectures = Lecture.objects.all().filter(
            begin__date=today, begin__lte=today_time)
        queryset = StudentsAttendLectures.objects.filter(student=instance, lecture__in=lectures).annotate(time=TruncTime(
            'lecture__begin')).values('id', 'lecture', 'present', 'time', course_id=F('lecture__course'), code=F('lecture__course__course__code'))
    ...
    def get_attendance(self, course_id, instance):
        ...
        today = datetime.datetime.now().date()
        lectures = Lecture.objects.all().filter(
            course__id=course_id, begin__date__lte=today)
        att = StudentsAttendLectures.objects.filter(
            student=instance, lecture__in=lectures).values('present')
...
class TeacherSerializer(serializers.ModelSerializer):
    ...
    def get_courses(self, instance):
        ...
        Gets the list of courses the student is enrolled in
        """
        queryset = list(TeachersTeachCourses.objects.filter(teacher=instance).values(
            'id', 'course__code', 'course__name', 'student_code', 'ta_code').annotate(students_count=Count('students')))
        return queryset
